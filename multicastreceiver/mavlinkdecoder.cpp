...
{
    qDebug() << "Create MAVLinkDecoder: " << this;
...
{
    qDebug() << "Destroy MAVLinkDecoder: " << this;
}
...
QList<QPair<QString,QVariant> > MAVLinkDecoder::receiveMessage(mavlink_message_t message)
{
   // Q_UNUSED(link);
    ...
    if (message.msgid == MAVLINK_MSG_ID_SYSTEM_TIME)
    {
        ...
        onboardToGCSUnixTimeOffsetAndDelay[message.sysid] = static_cast<qint64>(groundTimeMilliseconds() - (timebase.time_unix_usec+500)/1000);
    }
    else
    {
...
{
  //  //Interface *// = //Manager::instance()->get//ForId(msg->sysid);
    bool localemit = true;
//    if (!//)
//    {
//        //No active // for the incomign message.
//        localemit = true;
//    }
    ...
    case MAVLINK_TYPE_CHAR:
        ...
        else
        {
            ...
            {
                ////->valueChangedRec(msg->sysid,name,unit,b,time);
            }
            retval.second = b;
        }
        break;
    case MAVLINK_TYPE_UINT8_T:
        if (messageInfo[msgid].fields[fieldid].array_length > 0)
        {
            ...
            for (unsigned int j = 0; j < messageInfo[msgid].fields[fieldid].array_length; ++j)
            {
                ...
                {
                   // //->valueChangedRec(msg->sysid, QString("%1.%2").arg(name).arg(j), fieldType, nums[j], time);
                }
            }
        }
        else
        {
            ...
            {
                ////->valueChangedRec(msg->sysid, name, fieldType, u, time);
            }
            retval.second = u;
        }
        break;
    case MAVLINK_TYPE_INT8_T:
        ...
        else
        {
            ...
            {
                //->valueChangedRec(msg->sysid, name, fieldType, n, time);
            }
            retval.second = n;
        }
        break;
    ...
    case MAVLINK_TYPE_FLOAT:
        if (messageInfo[msgid].fields[fieldid].array_length > 0)
        {
            ...
            for (unsigned int j = 0; j < messageInfo[msgid].fields[fieldid].array_length; ++j)
            {
                ...
                {
                    //->valueChangedRec(msg->sysid, QString("%1.%2").arg(name).arg(j), fieldType, (float)(nums[j]), time);
                }
            }
        }
        else
        {
            ...
            {
                //->valueChangedRec(msg->sysid, name, fieldType, f, time);
            }
            retval.second = f;
        }
        break;
    ...
    case MAVLINK_TYPE_UINT64_T:
        if (messageInfo[msgid].fields[fieldid].array_length > 0)
        {
            ...
            for (unsigned int j = 0; j < messageInfo[msgid].fields[fieldid].array_length; ++j)
            {
                ...
                {
                    //->valueChangedRec(msg->sysid, QString("%1.%2").arg(name).arg(j), fieldType, (quint64) nums[j], time);
                }
            }
        }
        else
        {
            ...
            {
                //->valueChangedRec(msg->sysid, name, fieldType, (quint64) n, time);
            }
            retval.second = n;
        }
        break;
    case MAVLINK_TYPE_INT64_T:
        if (messageInfo[msgid].fields[fieldid].array_length > 0)
        {
            ...
            for (unsigned int j = 0; j < messageInfo[msgid].fields[fieldid].array_length; ++j)
            {
                ...
                {
                    //->valueChangedRec(msg->sysid, QString("%1.%2").arg(name).arg(j), fieldType, (qint64) nums[j], time);
                }
            }
        }
        else
        {
            ...
            {
                //->valueChangedRec(msg->sysid, name, fieldType, (qint64) n, time);
            }
            retval.second = n;
        }
        break;
...
quint64 MAVLinkDecoder::getUnixTimeFromMs(int systemID, quint64 time)
{
    ...
    if (time == 0)
    {
        ret = groundTimeMilliseconds() - onboardToGCSUnixTimeOffsetAndDelay.value(systemID,0);
    }
...
#endif
    {
        if (!onboardTimeOffset.contains(systemID) || (time < firstOnboardTime.value(systemID,0)-100))
        {
            ...
            onboardTimeOffset.insert(systemID,groundTimeMilliseconds() - time);
        }
